...
let strongest components =
  let rec loop tabu strongest current = function
      []    -> max (strength current) strongest
    | cs  -> match List.find cs (matching (Tabu.mem tabu) current) with
               None   -> max (strength current) strongest
  ...
  |> List.fold ~init:0 ~f:(fun strong start ->
